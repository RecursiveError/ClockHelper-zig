...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub const I2SClockSourceList = enum {
            ...
            pub fn to_enum(self: @This()) anyerror!RCC_SW {
                return switch (self) {
                    .RCC_I2SAPBCLKSOURCE_PLLSRC => .PLL1_P,
                    .RCC_I2SAPBCLKSOURCE_EXT => .HSE,
                    .RCC_I2SAPBCLKSOURCE_PLLR => .HSI,
                };
            }
            pub fn from_enum(item: RCC_SW) anyerror!@This() {
                return switch (item) {
                    .PLL1_P => .RCC_I2SAPBCLKSOURCE_PLLSRC,
                    .HSE => .RCC_I2SAPBCLKSOURCE_EXT,
                    .HSI => .RCC_I2SAPBCLKSOURCE_PLLR,
                };
            }
        };
        ...
        pub const RCC_TIM_PRescaler_SelectionList = enum {
            ...
            pub fn to_enum(self: @This()) anyerror!RCC_MCOPRE {
                return switch (self) {
                    .RCC_TIMPRES_ACTIVATED => .Div4,
                    .RCC_TIMPRES_DESACTIVATED => .Div1,
                };
            }
            pub fn from_enum(item: RCC_MCOPRE) anyerror!@This() {
                return switch (item) {
                    .Div4 => .RCC_TIMPRES_ACTIVATED,
                    .Div1 => .RCC_TIMPRES_DESACTIVATED,
                    else => return comptime_fail_or_error(
                        error.UnmappedListItem,
                        "{s} - {s} cannot be mapped to {s}List",
                        .{ "RCC_MCOPRE", @tagName(item), "RCC_TIM_PRescaler_Selection" },
                    ),
                };
            }
        };
        ...
        pub const ExtraConfig = struct {
            ...
            RCC_TIM_PRescaler_Selection: ?RCC_MCOPRE = null,
            HSE_Timout: ?u32 = null,
            LSE_Timout: ?u32 = null,
            I2SEnable: ?I2SEnableList = null,
            EnbaleCSS: ?EnbaleCSSList = null,
        ...
        pub const OutputConfig = struct {
            ...
            RCC_TIM_PRescaler_Selection: RCC_MCOPRE,
            HSE_Timout: u32,
            LSE_Timout: u32,
        ...
        pub fn get_cubemx_clocks(config: CubeMXConfig, comptime patch_logs: bool) anyerror!CubemxTreeOutput {
            ...
            var I2SSourceIsPllSRC: bool = false; //semaphore for processing, not an actual flag in the output;
            ...
            const I2SClockSourceValue: I2SClockSourceList = blk: {
                ...
                break :blk user_val orelse {
                    I2SSourceIsPll = true;
                    break :blk .RCC_I2SAPBCLKSOURCE_PLLR;
                };
            };
            ...
            const LSIUsedValue: u1 = blk: {
                if ((LPTIM1SOURCELSI and config.flags.LPTIMUsed_ForRCC) or (config.flags.SEM2RCC_LSI_REQUIRED_TIM5 and config.flags.TIM5 and config.flags.Semaphore_input_Channel4TIM5) or config.flags.IWDGUsed_ForRCC or ((check_ref(@TypeOf(RCC_RTC_Clock_SourceVirtualValue), RCC_RTC_Clock_SourceVirtualValue, .RCC_RTCCLKSOURCE_LSI, .@"=")) and (config.flags.RTCUsed_ForRCC))) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            const LSEUsedValue: u1 = blk: {
                if ((LPTIM1SOURCELSE and config.flags.LPTIMUsed_ForRCC) or (config.flags.SEM2RCC_LSE_REQUIRED_TIM5 and config.flags.TIM5 and config.flags.Semaphore_input_Channel4TIM5) or ((check_ref(@TypeOf(RCC_MCO1SourceValue), RCC_MCO1SourceValue, .RCC_MCO1SOURCE_LSE, .@"=")) and config.flags.MCO1Config) or ((check_ref(@TypeOf(RCC_RTC_Clock_SourceVirtualValue), RCC_RTC_Clock_SourceVirtualValue, .RCC_RTCCLKSOURCE_LSE, .@"=")) and config.flags.RTCUsed_ForRCC)) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            I2SSrc.parents = switch (I2SClockSourceValue) {
                .RCC_I2SAPBCLKSOURCE_PLLR => &.{&PLLR},
                .RCC_I2SAPBCLKSOURCE_PLLSRC => &.{&PLLSource},
                .RCC_I2SAPBCLKSOURCE_EXT => &.{&I2S_CKIN},
            };
            ...
            const VCOQOutputFreq_ValueValue = PLL48CLK.get_as_ref();
            ignore_value(VCOQOutputFreq_ValueValue);
            if (config.flags.RNGUsed_ForRCC) {
                PLL48CLK.limit = .{
                    .min = null,
                    .max = 7.5e7,
            ...
            const VCOROutputFreq_ValueValue = PLLRCLK.get_as_ref();
            ignore_value(VCOROutputFreq_ValueValue);
            if (I2SSourceIsPll) {
                PLLRCLK.limit = .{
                    .min = 2.4e7,
                    .max = 1e8,
                    .main_expr = "I2SSourceIsPll",
                    .main_dialog = "PLL not used",
                };
            }
