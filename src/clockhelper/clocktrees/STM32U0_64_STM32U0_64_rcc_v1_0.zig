...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub fn get_cubemx_clocks(config: CubeMXConfig, comptime patch_logs: bool) anyerror!CubemxTreeOutput {
            ...
            if (((RNGClockisPll and config.flags.RNG_Used) or (TIM15SOURCEPll and true) or (TIM1SOURCEPll and true) or (config.flags.USB_Used and check_MCU("USBCLKSOURCE_PLL"))) and scale1) {
                PLLQoutput.limit = .{
                    .min = null,
                    .max = 5.6e7,
                };
            } else if (((RNGClockisPll and config.flags.RNG_Used) or (TIM15SOURCEPll and true) or (TIM1SOURCEPll and true) or (config.flags.USB_Used and check_MCU("USBCLKSOURCE_PLL"))) and scale2) {
                PLLQoutput.limit = .{
                    .min = null,
                    .max = 1.9e7,
                };
            ...
            const PLLUsedValue: u1 = blk: {
                if ((config.flags.ADC1_Used and ADCSourcePllp) or (true and TIM15SOURCEPll) or (true and TIM1SOURCEPll) or (SysSourcePLL) or (MCOSourcePLL and config.flags.MCOConfig) or (MCO2SourcePLL and config.flags.MCO2Config) or (RNGClockisPll and config.flags.RNG_Used)) {
                    break :blk 1;
                }
                break :blk 0;
            };
