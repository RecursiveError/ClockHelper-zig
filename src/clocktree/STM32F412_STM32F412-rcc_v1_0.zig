...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            std.mem.doNotOptimizeAway(PLLI2SSourceValue);
            ...
            std.mem.doNotOptimizeAway(PLLI2SMValue);
            ...
            std.mem.doNotOptimizeAway(FMPI2C1SelectionValue);
            ...
            const PLLI2SQoutputFreq_ValueValue: ?f32 = blk: {
                break :blk 96000000;
            };

            std.mem.doNotOptimizeAway(PLLI2SQoutputFreq_ValueValue);
            ...
            std.mem.doNotOptimizeAway(EnableI2S1Value);
            ...
            std.mem.doNotOptimizeAway(EnableI2S2Value);
            ...
            std.mem.doNotOptimizeAway(EnableFMPI2C1Value);
            ...
            const PLLI2SSRC_clk_value = PLLI2SSourceValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                \\Error on Clock {s} | expr: {s} diagnostic: {s}
                \\Clock is active but the reference value {s} is null
                \\note: check the flags and configurations associated with this clock
                \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
