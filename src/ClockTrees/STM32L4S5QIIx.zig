...
pub const ClockTree = struct {
    ...
    pub fn init(comptime config: Config) Self {
        ...
        const AHBOutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 120000000), .min = 0 },
        };
        ...
        const ADCoutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 120000000), .min = 1400000 },
        };
    ...
    pub fn init_runtime_tree(self: *Self, alloc: std.mem.Allocator) !void {
        ...
        self.PLLM1.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSource,
        });
        self.PLLM2.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSource,
        });
        self.PLLM3.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSource,
        });
        ...
        self.PLLSAI2Q.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSAI2N,
        });
        self.PLLSAI2Qoutput.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSAI2Q,
        });
        self.PLLSAI2R.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSAI2N,
        });
        self.PLLSAI2Routput.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSAI2R,
        });
        self.PLLSAI2RDIVIDER.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSAI2R,
        });
        self.LTDCCLK.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSAI2RDIVIDER,
        });
    }
    pub fn deinit_runtime_tree(self: *Self, alloc: std.mem.Allocator) void {
        ...
        alloc.free(self.PLLM1.parents.?);
        alloc.free(self.PLLM2.parents.?);
        alloc.free(self.PLLM3.parents.?);
        ...
        alloc.free(self.PLLSAI2Q.parents.?);
        alloc.free(self.PLLSAI2Qoutput.parents.?);
        alloc.free(self.PLLSAI2R.parents.?);
        alloc.free(self.PLLSAI2Routput.parents.?);
        alloc.free(self.PLLSAI2RDIVIDER.parents.?);
        alloc.free(self.LTDCCLK.parents.?);
    }
    pub fn runtime_apply(self: *Self, config: Config) error{InvalidConfig}!void {
        ...
        self.PLLSAI2Q.nodetype = PLLSAI2Qval;
        const PLLSAI2Qoutputval = ClockNodeTypes{ .output = null };
        self.PLLSAI2Qoutput.nodetype = PLLSAI2Qoutputval;
        const PLLSAI2Rval = ClockNodeTypes{ .div = .{
            .value = inner: {
                if (config.PLLSAI2R) |val| {
                    break :inner val.get();
        ...
        self.PLLSAI2R.nodetype = PLLSAI2Rval;
        const PLLSAI2Routputval = ClockNodeTypes{ .output = null };
        self.PLLSAI2Routput.nodetype = PLLSAI2Routputval;
        const PLLSAI2RDIVIDERval = ClockNodeTypes{ .div = .{
            .value = inner: {
                if (config.PLLSAI2RDIVIDER) |val| {
                    break :inner val.get();
        ...
        self.PLLSAI2RDIVIDER.nodetype = PLLSAI2RDIVIDERval;
        const LTDCCLKval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 120000000), .min = 0 },
        };
        self.LTDCCLK.nodetype = LTDCCLKval;
        const HSE_Timoutval = ClockNodeTypes{
            .source = .{
                .value = if (config.HSE_Timout) |val| val.get() else 100,
                .limit = .{ .max = @min(1_000_000_000, 4294967295), .min = 1 },
