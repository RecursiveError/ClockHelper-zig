...
pub const ClockNode = struct {
    ...
    pub fn get_output(self: Self) !u32 {
...
pub fn check_ref(comptime T: type, val: T, to_check: T, comptime op: Opration) bool {
    switch (@typeInfo(T)) {
        .optional => {
            if (val == null and to_check == null) {
                return switch (op) {
                    .@"!=" => false,
                    else => true,
                };
            } else if ((val != null and to_check == null) or (to_check != null and val == null)) {
                return switch (op) {
                    .@"!=" => true,
                    else => false,
                };
            }
            return inner_check(@TypeOf(val.?), val.?, to_check.?, op);
        },
        else => {
            return inner_check(T, val, to_check, op);
        },
    }
}

fn inner_check(comptime T: type, val: T, to_check: T, comptime op: Opration) bool {
    return switch (op) {
        .@"=" => val == to_check,
        .@"!=" => val != to_check,
        .@">" => val > to_check,
...
pub fn math_op(comptime T: type, val: T, val2: T, comptime op: MathOpration, comptime owner: []const u8) !f32 {
    const t = comptime check_type(T) orelse @compileError("math_op used on invalid type:" ++ @typeName(T));

    if (val == null or val2 == null) {
        return comptime_fail_or_error(error.NullVal,
            \\fail to parser expr of ref {s}{s}...
            \\Null value.
        , .{ owner, @tagName(op) });
    }
    const op1 = val orelse unreachable;
    const op2 = val2 orelse unreachable;
        ...
        .@"+" => {
            ...
            if (!std.math.isFinite(ret)) {
                return comptime_fail_or_error(error.Overflow,
                    \\Error trying to evaluate the expression {}{s}{} on reference: {s}
                    \\Overflow.
                    \\
                    \\
                , .{ n1, @tagName(op), n2, owner });
            }
            return ret.@"0";
        },
        .@"-" => {
            const ret = n1 - n2;
            if (!std.math.isFinite(ret)) {
                return comptime_fail_or_error(error.Underflow,
                    \\Error trying to evaluate the expression {}{s}{} on reference: {s}
                    \\Underflow.
                    \\
                    \\
...
pub fn ignore_value(val: anytype) void {
    _ = val;
}
