    ...
    return struct {
        ...
        pub fn get_cubemx_clocks(config: CubeMXConfig, comptime patch_logs: bool) anyerror!CubemxTreeOutput {
            ...
            const PLLUsedValue: u1 = blk: {
                if (((SAI1SourcePLLP and (config.flags.SAI1Used_ForRCC))) or (SysSourcePLL) or ((check_ref(@TypeOf(RCC_MCO1SourceValue), RCC_MCO1SourceValue, .RCC_MCO1SOURCE_PLLCLK, .@"=")) and ((config.flags.Semaphore_input_Channel1TIM16 and config.flags.TIM16 and config.flags.SEM2RCC_MCO_REQUIRED_TIM16) or config.flags.MCOConfig)) or (CK48SourcePLLCLK and (config.flags.USBUsed_ForRCC or config.flags.RNGUsed_ForRCC or config.flags.SDMMC1Used_ForRCC))) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            if (((SAI1SourcePLLP and (config.flags.SAI1Used_ForRCC))) and scale1) {
                PLLPoutput.limit = .{
                    .min = 2.064e6,
                    .max = 8e7,
                    .main_expr = "((SAI1SourcePLLP&(SAI1Used_ForRCC))) & scale1",
                    .main_dialog = " ",
                };
                ...
                PLLPoutput.limit = .{
                    .min = 2.064e6,
                    .max = 2.6e7,
                    .main_expr = "((SAI1SourcePLLP&(SAI1Used_ForRCC))) & scale2",
                    .main_dialog = " ",
                };
            }
