...
pub const SYSCLKSourceList = enum {
    ...
    pub fn get(self: @This()) usize {
        return switch (self) {
            .RCC_SYSCLKSOURCE_LSI => 0,
            .RCC_SYSCLKSOURCE_HSE => 1,
            .RCC_SYSCLKSOURCE_HSI => 2,
            .RCC_SYSCLKSOURCE_LSE => 3,
        };
    }
};
...
pub const I2S1CLockSelectionList = enum {
    ...
    pub fn get(self: @This()) usize {
        return switch (self) {
            ...
            .RCC_I2S1CLKSOURCE_HSIKER => 1,
            .RCC_I2S1CLKSOURCE_EXT => 2,
        };
    }
};
...
pub const I2C1CLockSelectionList = enum {
    ...
    pub fn get(self: @This()) usize {
        return switch (self) {
            ...
            .RCC_I2C1CLKSOURCE_HSIKER => 2,
        };
    }
};
pub const FDCAN1CLockSelectionList = enum {
    RCC_FDCAN1CLKSOURCE_PCLK1,
    RCC_FDCAN1CLKSOURCE_HSIKER,
    RCC_FDCAN1CLKSOURCE_HSE,
    pub fn get(self: @This()) usize {
        return switch (self) {
            .RCC_FDCAN1CLKSOURCE_PCLK1 => 0,
            .RCC_FDCAN1CLKSOURCE_HSIKER => 1,
...
pub const FDCAN1EnableList = enum {
    true,
    false,
};
...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub const Config = struct {
            ...
            FDCAN1CLockSelection: ?FDCAN1CLockSelectionList = null,
        ...
        pub const Config_Output = struct {
            ...
            HSIKERCLKDivider: ?HSIKERCLKDividerList = null, //from RCC Clock Config
            ...
            SYSDIV: ?SYSDIVList = null, //from RCC Clock Config
            ...
            FDCAN1CLockSelection: ?FDCAN1CLockSelectionList = null, //from RCC Clock Config
            ...
            FDCAN1Enable: ?FDCAN1EnableList = null, //from extra RCC references
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            var FDCAN1SourcePCLK1: bool = false;
            var FDCAN1SourceHSI: bool = false;
            var FDCAN1SourceHSE: bool = false;
            ...
            var APBTimFreq_ValueLimit: Limit = .{};
            //Ref Values
            ...
            const HSIKERCLKDividerValue: ?HSIKERCLKDividerList = blk: {
                ...
                break :blk conf_item orelse .RCC_HSIKER_DIV3;
            };

            std.mem.doNotOptimizeAway(HSIKERCLKDividerValue);
            ...
            const SYSCLKSourceValue: ?SYSCLKSourceList = blk: {
                ...
                if (conf_item) |item| {
                    switch (item) {
                        ...
                        .RCC_SYSCLKSOURCE_HSIUSB48 => SysSourceHSI48 = true,
                    }
                }
            ...
            std.mem.doNotOptimizeAway(SYSDIVValue);
            ...
            const FDCAN1CLockSelectionValue: ?FDCAN1CLockSelectionList = blk: {
                const conf_item = config.FDCAN1CLockSelection;
                if (conf_item) |item| {
                    switch (item) {
                        .RCC_FDCAN1CLKSOURCE_PCLK1 => FDCAN1SourcePCLK1 = true,
                        .RCC_FDCAN1CLKSOURCE_HSIKER => FDCAN1SourceHSI = true,
                        .RCC_FDCAN1CLKSOURCE_HSE => FDCAN1SourceHSE = true,
                    }
                }

                break :blk conf_item orelse .RCC_FDCAN1CLKSOURCE_PCLK1;
            };

            std.mem.doNotOptimizeAway(FDCAN1CLockSelectionValue);
            const FDCAN1Freq_ValueValue: ?f32 = blk: {
                break :blk 4000000;
            };

            std.mem.doNotOptimizeAway(FDCAN1Freq_ValueValue);
            ...
            const RCC_MCO2SourceValue: ?RCC_MCO2SourceList = blk: {
                ...
                if (conf_item) |item| {
                    switch (item) {
                        ...
                        .RCC_MCO2SOURCE_HSI48 => MCO2SourceHSI48 = true,
                    }
                }
            ...
            const LSEUsedValue: ?f32 = blk: {
                if ((SysSourceLse) or (LSCOSSourceLSE and config.flags.LSCOConfig) or (USART1SourceLSE and config.flags.USART1_Used) or (MCOSourceLSE and config.flags.MCOConfig) or (MCO2SourceLSE and config.flags.MCO2Config) or (RTCSourceLSE and config.flags.RTC_Used)) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            const FDCAN1EnableValue: ?FDCAN1EnableList = blk: {
                if (config.flags.FDCAN1_Used) {
                    const item: FDCAN1EnableList = .true;
                    break :blk item;
                }
                const item: FDCAN1EnableList = .false;
                break :blk item;
            };

            std.mem.doNotOptimizeAway(FDCAN1EnableValue);
            ...
            var FDCAN1Mult = ClockNode{
                .name = "FDCAN1Mult",
                .nodetype = .off,
                .parents = &.{},
            };

            var FDCAN1output = ClockNode{
                .name = "FDCAN1output",
                .nodetype = .off,
                .parents = &.{},
            };
            ...
            SYSDIV.nodetype = .div;
            SYSDIV.value = SYSDIV_clk_value.get();
            ...
            if (check_MCU("FDCAN1_Exist")) {
                if (check_ref(@TypeOf(FDCAN1EnableValue), FDCAN1EnableValue, .true, .@"=")) {
                    const FDCAN1Mult_clk_value = FDCAN1CLockSelectionValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                        \\Error on Clock {s} | expr: {s} diagnostic: {s}
                        \\Clock is active but the reference value {s} is null
                        \\note: check the flags and configurations associated with this clock
                        \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
                    , .{
                        ...
                        "FDCAN1_Exist",
                        "No Extra Log",
                        "FDCAN1CLockSelection",
                    });
                    const FDCAN1Multparents = [_]*const ClockNode{
                        &APBPrescaler,
                        &HSIKER,
                        &HSEOSC,
                    };
                    FDCAN1Mult.nodetype = .multi;
                    FDCAN1Mult.parents = &.{FDCAN1Multparents[FDCAN1Mult_clk_value.get()]};
                }
            }
            if (check_MCU("FDCAN1_Exist")) {
                if (check_ref(@TypeOf(FDCAN1EnableValue), FDCAN1EnableValue, .true, .@"=")) {
                    FDCAN1output.nodetype = .output;
                    FDCAN1output.parents = &.{&FDCAN1Mult};
                }
            }
            ...
            TimPrescOut1.limit = APBTimFreq_ValueLimit;
            ...
            out.FDCAN1Mult = try FDCAN1Mult.get_output();
            out.FDCAN1output = try FDCAN1output.get_output();
            ...
            ref_out.FDCAN1CLockSelection = FDCAN1CLockSelectionValue;
            ...
            ref_out.FDCAN1Enable = FDCAN1EnableValue;
