    ...
    return struct {
        ...
        pub fn get_cubemx_clocks(config: CubeMXConfig, comptime patch_logs: bool) anyerror!CubemxTreeOutput {
            ...
            const LPTIM3EnableValue: LPTIM3EnableList = blk: {
                if (config.flags.LPTIM3_Used and check_ref(@TypeOf(CodegenConfigPeriphValue), CodegenConfigPeriphValue, .true, .@"=")) {
                    break :blk .true;
                }
                break :blk .false;
            };
            ...
            const LPUART3EnableValue: LPUART3EnableList = blk: {
                if (config.flags.LPUART3_Used and check_ref(@TypeOf(CodegenConfigPeriphValue), CodegenConfigPeriphValue, .true, .@"=")) {
                    break :blk .true;
                }
                break :blk .false;
            };
            ...
            const USBEnableValue: USBEnableList = blk: {
                if (config.flags.USB_Used and check_ref(@TypeOf(CodegenConfigPeriphValue), CodegenConfigPeriphValue, .true, .@"=")) {
                    break :blk .true;
                }
                break :blk .false;
            };
            ...
            if (((TIM15SOURCEPll and config.flags.TIM15) or (TIM1SOURCEPll and config.flags.TIM1) or ((config.flags.USB_Used or config.flags.RNG_Used) and config.flags.USBCLKSOURCE_PLL)) and scale1) {
                PLLQoutput.limit = .{
                    .min = null,
                    .max = 5.6e7,
                    .main_expr = "((TIM15SOURCEPll & TIM15) | (TIM1SOURCEPll & TIM1) | ((USB_Used|RNG_Used) & USBCLKSOURCE_PLL)) & scale1",
                    .main_dialog = " ",
                };
            } else if (((TIM15SOURCEPll and config.flags.TIM15) or (TIM1SOURCEPll and config.flags.TIM1) or ((config.flags.USB_Used or config.flags.RNG_Used) and config.flags.USBCLKSOURCE_PLL)) and scale2) {
                PLLQoutput.limit = .{
                    .min = null,
                    .max = 1.9e7,
                    .main_expr = "((TIM15SOURCEPll & TIM15) | (TIM1SOURCEPll & TIM1) | ((USB_Used|RNG_Used) & USBCLKSOURCE_PLL)) & scale2",
                    .main_dialog = " ",
                };
            }

            const PLLUsedValue: u1 = blk: {
                if ((config.flags.ADC1_Used and ADCSourcePllp) or (config.flags.TIM15 and TIM15SOURCEPll) or (config.flags.TIM1 and TIM1SOURCEPll) or (SysSourcePLL) or (MCOSourcePLL and config.flags.MCOConfig) or (MCO2SourcePLL and config.flags.MCO2Config) or (USBCLKSOURCE_PLLQ and (config.flags.RNG_Used or config.flags.USB_Used))) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            if (!(check_ref(@TypeOf(LPUART3EnableValue), LPUART3EnableValue, .true, .@"="))) {
                LPUART3Mult.nodetype = .off;
            }
            if (!(check_ref(@TypeOf(LPUART3EnableValue), LPUART3EnableValue, .true, .@"="))) {
                LPUART3output.nodetype = .off;
            }
