...
pub const ClockTree = struct {
    ...
    pub fn init(comptime config: Config) Self {
        ...
        const SDADCPrescval = ClockNodeTypes{ .div = .{
            .value = inner: {
                if (config.SDADCPresc) |val| {
                    break :inner val.get();
                } else {
                    break :inner 2;
                }
        ...
        const SDADCoutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 6000000), .min = 500000 },
        };
        ...
        const ADCPrescval = ClockNodeTypes{ .div = .{
            .value = inner: {
                if (config.ADCPresc) |val| {
                    break :inner val.get();
                } else {
                    break :inner 2;
                }
    ...
    pub fn init_runtime_tree(self: *Self, alloc: std.mem.Allocator) !void {
        ...
        self.SDADCPresc.parents = try alloc.dupe(*const ClockNode, &.{
            &self.SysCLKOutput,
        });
        self.SDADCoutput.parents = try alloc.dupe(*const ClockNode, &.{
            &self.SDADCPresc,
        });
        ...
        self.ADCPresc.parents = try alloc.dupe(*const ClockNode, &.{
            &self.APB2Prescaler,
        });
    ...
    pub fn deinit_runtime_tree(self: *Self, alloc: std.mem.Allocator) void {
        ...
        alloc.free(self.SDADCPresc.parents.?);
        alloc.free(self.SDADCoutput.parents.?);
        ...
        alloc.free(self.ADCPresc.parents.?);
    ...
    pub fn runtime_apply(self: *Self, config: Config) error{InvalidConfig}!void {
        ...
        self.SDADCPresc.nodetype = SDADCPrescval;
        const SDADCoutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 6000000), .min = 500000 },
        };
        self.SDADCoutput.nodetype = SDADCoutputval;
        const PWROutputval = ClockNodeTypes{ .output = null };
        ...
        self.ADCPresc.nodetype = ADCPrescval;
        const ADCoutputval = ClockNodeTypes{ .output = null };
