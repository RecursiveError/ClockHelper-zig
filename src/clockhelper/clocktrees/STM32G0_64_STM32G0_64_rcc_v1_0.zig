    ...
    return struct {
        ...
        pub const OutputConfig = struct {
            ...
            pub fn from_cubemx_output_config(cubemx_config: CubeMXOutputConfig) !OutputConfig {
                return OutputConfig{
                    ...
                    .HSI_DIV8 = cubemx_config.HSI_DIV8,
        ...
        pub fn get_cubemx_clocks(config: CubeMXConfig, comptime patch_logs: bool) anyerror!CubemxTreeOutput {
            ...
            const TIM1EnableValue: TIM1EnableList = blk: {
                if (config.flags.TIM1 and check_ref(@TypeOf(CodegenConfigPeriphValue), CodegenConfigPeriphValue, .true, .@"=") and !check_MCU("STM32G0x0_Value_line")) {
                    break :blk .true;
                }
                break :blk .false;
            };
            ...
            if (check_MCU("RNG_Exist")) {
                ...
                HSI_DIV8.value = @floatFromInt(HSI_DIV8Value);
            ...
            if (check_MCU("RNG_Exist")) {
                ...
                CK48Mult.parents = switch (RNGCLockSelectionValue) {
                    ...
                    .RCC_RNGCLKSOURCE_HSI_DIV8 => &.{&HSI_DIV8},
                };
            }
            // ======= NODE RNGoutput ======
            if (check_MCU("RNG_Exist")) {
            ...
            if (scale1 and ((config.flags.ADC1_USED and config.flags.ADCUsed_ForRCC and ADCSourcePllp) or (I2S1SourcePll and config.flags.I2S1Used_ForRCC))) {
                PLLPoutput.limit = .{
                    .min = 3.09e6,
                    .max = 1.22e8,
                    .main_expr = "scale1 & ((ADC1_USED & ADCUsed_ForRCC & ADCSourcePllp) | (I2S1SourcePll & I2S1Used_ForRCC))",
                    .main_dialog = " ",
                };
            } else {
                PLLPoutput.limit = .{
            ...
            if (scale1 and ((RNGClockisPll and config.flags.RNGUsed_ForRCC) or (TIM1SOURCEPll and config.flags.TIM1Used_ForRCC) or (TIM1SOURCEPll and config.flags.TIM1))) {
                PLLQoutput.limit = .{
                    .min = 1.2e7,
                    .max = 1.28e8,
                    .main_expr = "scale1 & ((RNGClockisPll & RNGUsed_ForRCC ) | (TIM1SOURCEPll & TIM1Used_ForRCC) | (TIM1SOURCEPll & TIM1))",
                    .main_dialog = " ",
                };
            } else {
                PLLQoutput.limit = .{
            ...
            const PLLUsedValue: u1 = blk: {
                if ((I2S1SourcePll and config.flags.I2S1Used_ForRCC) or (config.flags.ADC1_USED and ADCSourcePllp and config.flags.ADCUsed_ForRCC) or (config.flags.TIM1 and TIM1SOURCEPll) or (SysSourcePLL) or ((check_ref(@TypeOf(RCC_MCO1SourceValue), RCC_MCO1SourceValue, .RCC_MCO1SOURCE_PLLCLK, .@"=")) and ((config.flags.Semaphore_input_Channel1TIM17 and config.flags.TIM17 and config.flags.SEM2RCC_MCO_REQUIRED_TIM17) or config.flags.MCOConfig)) or (RNGClockisPll and (config.flags.RNGUsed_ForRCC))) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            if (scale1 and ((check_ref(@TypeOf(PLLUsedValue), PLLUsedValue, 1, .@"=") and SysSourcePLL) or (MCOSourcePLL and config.flags.MCOConfig))) {
                PLLCLK.limit = .{
                    .min = 1.2e7,
                    .max = 6.4e7,
                    .main_expr = "scale1 & ((PLLUsed=1 & SysSourcePLL) | (MCOSourcePLL & MCOConfig))",
                    .main_dialog = " ",
                };
            } else if (check_ref(@TypeOf(PLLUsedValue), PLLUsedValue, 1, .@"=") or (MCOSourcePLL and config.flags.MCOConfig) or scale2) {
                PLLCLK.limit = .{
                    .min = 1.2e7,
                    .max = 1.6e7,
                    .main_expr = "PLLUsed=1 | (MCOSourcePLL & MCOConfig) | scale2",
                    .main_dialog = "PLL is limited to 16 MHz when: \n - is used but is not the system clock source.\n - scale2.",
                };
            }
                        ...
                        switch (val) {
                            ...
                            .FLASH_LATENCY_2 => {},
                                ...
                                , .{
                                    ...
                                    if (patch_logs) "WS1" else "FLASH_LATENCY_1",
                                    if (patch_logs) "WS2" else "FLASH_LATENCY_2",
                                });
                            },
                        }
                    }
                        ...
                        switch (val) {
                            ...
                            .FLASH_LATENCY_1 => {},
                                ...
                                , .{
                                    ...
                                    "",
                                    if (patch_logs) @tagName(try val.to_enum()) else @tagName(val),
                                    if (patch_logs) "WS0" else "FLASH_LATENCY_0",
                                    if (patch_logs) "WS1" else "FLASH_LATENCY_1",
                                });
                            },
                        }
            ...
            if (!(check_ref(@TypeOf(RNGPLLEnableValue), RNGPLLEnableValue, .true, .@"=") or check_ref(@TypeOf(TIM1PLLEnableValue), TIM1PLLEnableValue, .true, .@"="))) {
