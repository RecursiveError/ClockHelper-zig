    ...
    return struct {
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            const PLLSAI1SourceValue: ?PLLSAI1SourceList = blk: {
                ...
                break :blk conf_item orelse .RCC_PLLSAI1SOURCE_MSI;
            };

            std.mem.doNotOptimizeAway(PLLSAI1SourceValue);
            const PLLSAI2SourceValue: ?PLLSAI2SourceList = blk: {
                ...
                break :blk conf_item orelse .RCC_PLLSAI2SOURCE_MSI;
            };

            std.mem.doNotOptimizeAway(PLLSAI2SourceValue);
            ...
            std.mem.doNotOptimizeAway(PLLSAI1MValue);
            ...
            std.mem.doNotOptimizeAway(PLLSAI2MValue);
            ...
            const DFSDMCLockSelectionValue: ?DFSDMCLockSelectionList = blk: {
                ...
                break :blk conf_item orelse .RCC_DFSDM1CLKSOURCE_PCLK2;
            };
            ...
            std.mem.doNotOptimizeAway(DFSDMAudioCLockSelectionValue);
            ...
            std.mem.doNotOptimizeAway(DFSDMAudioEnableValue);
            ...
            const PLLSAI1Source_clk_value = PLLSAI1SourceValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                \\Error on Clock {s} | expr: {s} diagnostic: {s}
                \\Clock is active but the reference value {s} is null
                \\note: check the flags and configurations associated with this clock
                \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            const PLLSAI2Source_clk_value = PLLSAI2SourceValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                \\Error on Clock {s} | expr: {s} diagnostic: {s}
                \\Clock is active but the reference value {s} is null
                \\note: check the flags and configurations associated with this clock
                \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            const PLLSAI1M_clk_value = PLLSAI1MValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                \\Error on Clock {s} | expr: {s} diagnostic: {s}
                \\Clock is active but the reference value {s} is null
                \\note: check the flags and configurations associated with this clock
                \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            const PLLSAI2M_clk_value = PLLSAI2MValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                \\Error on Clock {s} | expr: {s} diagnostic: {s}
                \\Clock is active but the reference value {s} is null
                \\note: check the flags and configurations associated with this clock
                \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
