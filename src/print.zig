...
pub fn print_clock_configs(comptime clock: anytype) [calc_print_clock_configs(clock)]u8 {
    comptime {
        ...
        const final = comp_buffer[0..index];
        return final.*;
    }
}

pub fn calc_print_clock_configs(comptime clock: anytype) usize {
    comptime {
        var comp_buffer: [10000:0]u8 = undefined;
        var index = 0;
        const info = @typeInfo(@TypeOf(clock));
