...
pub const DFSDM2AudioSelectionConf = enum {
...
pub const SAI1ACLockSourceSelectionConf = enum {
...
pub const PLLDivRConf = enum(u32) {
    _,
    pub fn get(num: @This()) f32 {
        const val: u32 = @intFromEnum(num);
        return @as(f32, @floatFromInt(val));
...
pub const SAI1BCLockSourceSelectionConf = enum {
...
pub const PLLI2SDivRConf = enum(u32) {
    _,
    pub fn get(num: @This()) f32 {
        const val: u32 = @intFromEnum(num);
        return @as(f32, @floatFromInt(val));
...
pub const Config = struct {
    ...
    DFSDM2AudioMult: ?DFSDM2AudioSelectionConf = null,
    SAI1AMult: ?SAI1ACLockSourceSelectionConf = null,
    SAI1APrescaler: ?PLLDivRConf = null,
    SAI1BMult: ?SAI1BCLockSourceSelectionConf = null,
    SAI1BPrescaler: ?PLLI2SDivRConf = null,
    LPTimerMult: ?LPTIM1CLockSelectionConf = null,
    PLLN: ?PLLNConf = null,
    PLLP: ?PLLPConf = null,
    PLLQ: ?PLLQConf = null,
    PLLR: ?PLLRConf = null,
...
pub const ConfigWithRef = struct {
    ...
    DFSDM2AudioSelection: ?DFSDM2AudioSelectionConf = null,
    SAI1ACLockSourceSelection: ?SAI1ACLockSourceSelectionConf = null,
    PLLDivR: ?PLLDivRConf = null,
    SAI1BCLockSourceSelection: ?SAI1BCLockSourceSelectionConf = null,
    PLLI2SDivR: ?PLLI2SDivRConf = null,
    ...
    pub fn into_config(self: *const ConfigWithRef) Config {
        return .{
            ...
            .DFSDM2AudioMult = self.DFSDM2AudioSelection,
            .SAI1AMult = self.SAI1ACLockSourceSelection,
            .SAI1APrescaler = self.PLLDivR,
            .SAI1BMult = self.SAI1BCLockSourceSelection,
            .SAI1BPrescaler = self.PLLI2SDivR,
            .LPTimerMult = self.LPTIM1CLockSelection,
            .PLLN = self.PLLN,
            .PLLP = self.PLLP,
            .PLLQ = self.PLLQ,
            .PLLR = self.PLLR,
...
pub const ClockTree = struct {
    ...
    pub fn init(comptime config: Config) Self {
        ...
        const DFSDM2AudioMultval = ClockNodeTypes{
            .multi = inner: {
                if (config.DFSDM2AudioMult) |val| {
                    break :inner val.get();
                } else {
                    break :inner 0;
                }
        ...
        const DFSDM2Audiooutputval = ClockNodeTypes{ .output = null };
        const DFSDM2Audiooutput: ClockNode = .{
            .name = "DFSDM2Audiooutput",
            .nodetype = DFSDM2Audiooutputval,
            .parents = &[_]*const ClockNode{&DFSDM2AudioMult},
        };
        const SAI1APrescalerval = ClockNodeTypes{
            .div = .{
                .value = if (config.SAI1APrescaler) |val| val.get() else 1,
                .limit = .{ .max = @min(1_000_000_000, 32), .min = 1 },
            },
        };
        const SAI1APrescaler: ClockNode = .{
            .name = "SAI1APrescaler",
            .nodetype = SAI1APrescalerval,
            .parents = &[_]*const ClockNode{&PLLR},
        };
        const SAI1BPrescalerval = ClockNodeTypes{
            .div = .{
                .value = if (config.SAI1BPrescaler) |val| val.get() else 1,
                .limit = .{ .max = @min(1_000_000_000, 32), .min = 1 },
            },
        };
        const SAI1BPrescaler: ClockNode = .{
            .name = "SAI1BPrescaler",
            .nodetype = SAI1BPrescalerval,
            .parents = &[_]*const ClockNode{&PLLI2SR},
        };
        const SAI1AMultval = ClockNodeTypes{
            .multi = inner: {
                if (config.SAI1AMult) |val| {
                    break :inner val.get();
                } else {
                    break :inner 0;
                }
        ...
        const SAI1Aoutputval = ClockNodeTypes{ .output = null };
        const SAI1Aoutput: ClockNode = .{
            .name = "SAI1Aoutput",
            .nodetype = SAI1Aoutputval,
            .parents = &[_]*const ClockNode{&SAI1AMult},
        };
        const SAI1BMultval = ClockNodeTypes{
            .multi = inner: {
                if (config.SAI1BMult) |val| {
                    break :inner val.get();
                } else {
                    break :inner 0;
                }
        ...
        const SAI1Boutputval = ClockNodeTypes{ .output = null };
        const SAI1Boutput: ClockNode = .{
            .name = "SAI1Boutput",
            .nodetype = SAI1Boutputval,
            .parents = &[_]*const ClockNode{&SAI1BMult},
        };
        const LPTimerMultval = ClockNodeTypes{
            .multi = inner: {
                if (config.LPTimerMult) |val| {
                    break :inner val.get();
                } else {
                    break :inner 0;
                }
        ...
        const LPTimeroutputval = ClockNodeTypes{ .output = null };
        const LPTimeroutput: ClockNode = .{
            .name = "LPTimeroutput",
            .nodetype = LPTimeroutputval,
    ...
    pub fn init_runtime_tree(self: *Self, alloc: std.mem.Allocator) !void {
        ...
        self.DFSDM2AudioMult.parents = try alloc.dupe(*const ClockNode, &.{
            &self.I2S1Mult,
            &self.I2S2Mult,
        });
        self.DFSDM2Audiooutput.parents = try alloc.dupe(*const ClockNode, &.{
            &self.DFSDM2AudioMult,
        });
        self.SAI1AMult.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSource,
            &self.SAI1APrescaler,
            &self.SAI1BPrescaler,
            &self.I2S_CKIN,
        });
        self.SAI1Aoutput.parents = try alloc.dupe(*const ClockNode, &.{
            &self.SAI1AMult,
        });
        self.SAI1APrescaler.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLR,
        });
        self.SAI1BMult.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSource,
            &self.SAI1APrescaler,
            &self.SAI1BPrescaler,
            &self.I2S_CKIN,
        });
        self.SAI1Boutput.parents = try alloc.dupe(*const ClockNode, &.{
            &self.SAI1BMult,
        });
        self.SAI1BPrescaler.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLI2SR,
        });
        self.LPTimerMult.parents = try alloc.dupe(*const ClockNode, &.{
            &self.APB1Prescaler,
            &self.LSIRC,
            &self.HSIRC,
            &self.LSEOSC,
        });
        self.LPTimeroutput.parents = try alloc.dupe(*const ClockNode, &.{
            &self.LPTimerMult,
        });
    ...
    pub fn deinit_runtime_tree(self: *Self, alloc: std.mem.Allocator) void {
        ...
        alloc.free(self.DFSDM2AudioMult.parents.?);
        alloc.free(self.DFSDM2Audiooutput.parents.?);
        alloc.free(self.SAI1AMult.parents.?);
        alloc.free(self.SAI1Aoutput.parents.?);
        alloc.free(self.SAI1APrescaler.parents.?);
        alloc.free(self.SAI1BMult.parents.?);
        alloc.free(self.SAI1Boutput.parents.?);
        alloc.free(self.SAI1BPrescaler.parents.?);
        alloc.free(self.LPTimerMult.parents.?);
        alloc.free(self.LPTimeroutput.parents.?);
    ...
    pub fn runtime_apply(self: *Self, config: Config) error{InvalidConfig}!void {
        ...
        self.DFSDM2AudioMult.nodetype = DFSDM2AudioMultval;
        const DFSDM2Audiooutputval = ClockNodeTypes{ .output = null };
        self.DFSDM2Audiooutput.nodetype = DFSDM2Audiooutputval;
        const SAI1APrescalerval = ClockNodeTypes{
            .div = .{
                .value = if (config.SAI1APrescaler) |val| val.get() else 1,
                .limit = .{ .max = @min(1_000_000_000, 32), .min = 1 },
        ...
        self.SAI1APrescaler.nodetype = SAI1APrescalerval;
        const SAI1BPrescalerval = ClockNodeTypes{
            .div = .{
                .value = if (config.SAI1BPrescaler) |val| val.get() else 1,
                .limit = .{ .max = @min(1_000_000_000, 32), .min = 1 },
        ...
        self.SAI1BPrescaler.nodetype = SAI1BPrescalerval;
        const SAI1AMultval = ClockNodeTypes{
            .multi = inner: {
                if (config.SAI1AMult) |val| {
                    break :inner val.get();
        ...
        self.SAI1AMult.nodetype = SAI1AMultval;
        const SAI1Aoutputval = ClockNodeTypes{ .output = null };
        self.SAI1Aoutput.nodetype = SAI1Aoutputval;
        const SAI1BMultval = ClockNodeTypes{
            .multi = inner: {
                if (config.SAI1BMult) |val| {
                    break :inner val.get();
        ...
        self.SAI1BMult.nodetype = SAI1BMultval;
        const SAI1Boutputval = ClockNodeTypes{ .output = null };
        self.SAI1Boutput.nodetype = SAI1Boutputval;
        const LPTimerMultval = ClockNodeTypes{
            .multi = inner: {
                if (config.LPTimerMult) |val| {
                    break :inner val.get();
        ...
        self.LPTimerMult.nodetype = LPTimerMultval;
        const LPTimeroutputval = ClockNodeTypes{ .output = null };
        self.LPTimeroutput.nodetype = LPTimeroutputval;
        const PLLQoutputval = ClockNodeTypes{ .output = null };
