...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub const Config_Output = struct {
            ...
            Tim2Enable: ?Tim2EnableList = null, //from extra RCC references
            Tim3Enable: ?Tim3EnableList = null, //from extra RCC references
            Tim4Enable: ?Tim4EnableList = null, //from extra RCC references
            ...
            Tim20Enable: ?Tim20EnableList = null, //from extra RCC references
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            const SYSCLKFreq_VALUEValue: ?f32 = blk: {
                if ((TIM1SourcePLL and check_MCU("TIM1")) and (check_MCU("DIE422"))) {
                    const min: ?f32 = try math_op(@TypeOf(TIM1Freq_ValueValue), TIM1Freq_ValueValue, 2, .@"/", "SYSCLKFreq_VALUE", "TIM1Freq_Value", "2");
                    const max: ?f32 = null;
                    ...
                    const min: ?f32 = try math_op(@TypeOf(TIM1Freq_ValueValue), TIM1Freq_ValueValue, 4, .@"/", "SYSCLKFreq_VALUE", "TIM1Freq_Value", "4");
                    const max: ?f32 = null;
                    SYSCLKFreq_VALUELimit = .{
                        .min = min,
                        .max = max,
                    ...
                    const min: ?f32 = try math_op(@TypeOf(TIM8Freq_ValueValue), TIM8Freq_ValueValue, 2, .@"/", "SYSCLKFreq_VALUE", "TIM8Freq_Value", "2");
                    const max: ?f32 = null;
                    SYSCLKFreq_VALUELimit = .{
                        .min = min,
                        .max = max,
                    ...
                    const min: ?f32 = try math_op(@TypeOf(TIM8Freq_ValueValue), TIM8Freq_ValueValue, 4, .@"/", "SYSCLKFreq_VALUE", "TIM8Freq_Value", "4");
                    const max: ?f32 = null;
                    SYSCLKFreq_VALUELimit = .{
                        .min = min,
                        .max = max,
            ...
            std.mem.doNotOptimizeAway(TIM2SelectionValue);
            const TIM2Freq_ValueValue: ?f32 = blk: {
                break :blk 8000000;
            };

            std.mem.doNotOptimizeAway(TIM2Freq_ValueValue);
            ...
            std.mem.doNotOptimizeAway(TIM34SelectionValue);
            const TIM3Freq_ValueValue: ?f32 = blk: {
                break :blk 8000000;
            };

            std.mem.doNotOptimizeAway(TIM3Freq_ValueValue);
            ...
            std.mem.doNotOptimizeAway(TIM20SelectionValue);
            const TIM20Freq_ValueValue: ?f32 = blk: {
                break :blk 8000000;
            };

            std.mem.doNotOptimizeAway(TIM20Freq_ValueValue);
            ...
            std.mem.doNotOptimizeAway(Tim2EnableValue);
            ...
            std.mem.doNotOptimizeAway(Tim3EnableValue);
            ...
            std.mem.doNotOptimizeAway(Tim4EnableValue);
            ...
            std.mem.doNotOptimizeAway(Tim20EnableValue);
            ...
            if (check_MCU("ADC3_Exist")) {
                if (check_ref(@TypeOf(Tim2EnableValue), Tim2EnableValue, .true, .@"=")) {
                    const TIMMUX2_clk_value = TIM2SelectionValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                        \\Error on Clock {s} | expr: {s} diagnostic: {s}
                        \\Clock is active but the reference value {s} is null
                        \\note: check the flags and configurations associated with this clock
                        \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_MCU("ADC3_Exist")) {
                ...
                {
                    const TIMMUX3_clk_value = TIM34SelectionValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                        \\Error on Clock {s} | expr: {s} diagnostic: {s}
                        \\Clock is active but the reference value {s} is null
                        \\note: check the flags and configurations associated with this clock
                        \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(Tim20EnableValue), Tim20EnableValue, .true, .@"=")) {
                const TIMMUX20_clk_value = TIM20SelectionValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            ref_out.Tim2Enable = Tim2EnableValue;
            ref_out.Tim3Enable = Tim3EnableValue;
            ref_out.Tim4Enable = Tim4EnableValue;
            ...
            ref_out.Tim20Enable = Tim20EnableValue;
