...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            std.mem.doNotOptimizeAway(PLLI2SDivQValue);
            ...
            std.mem.doNotOptimizeAway(SAIClockSource_BlockAValue);
            ...
            std.mem.doNotOptimizeAway(PLLSAIDivRValue);
            ...
            std.mem.doNotOptimizeAway(PLLSAIDivQValue);
            ...
            std.mem.doNotOptimizeAway(SAIClockSource_BlockBValue);
            ...
            std.mem.doNotOptimizeAway(SAIAEnableValue);
            ...
            std.mem.doNotOptimizeAway(SAIBEnableValue);
            ...
            {
                const PLLI2SSAIDiv_clk_value = PLLI2SDivQValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(SAIAEnableValue), SAIAEnableValue, .true, .@"=")) {
                const SAI_AMuxSrc_clk_value = SAIClockSource_BlockAValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(LTDCEnableValue), LTDCEnableValue, .true, .@"=")) {
                const PLLSAILCDDiv_clk_value = PLLSAIDivRValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(SAIAEnableValue), SAIAEnableValue, .true, .@"=") or
                check_ref(@TypeOf(SAIBEnableValue), SAIBEnableValue, .true, .@"="))
            {
                const PLLSAISAIDiv_clk_value = PLLSAIDivQValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(SAIBEnableValue), SAIBEnableValue, .true, .@"=")) {
                const SAI_BMuxSrc_clk_value = SAIClockSource_BlockBValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
