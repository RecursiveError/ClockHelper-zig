...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub const Config_Output = struct {
            ...
            SWPMIEnable: ?SWPMIEnableList = null, //from extra RCC references
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            const SWPMI1CLockSelectionValue: ?SWPMI1CLockSelectionList = blk: {
                ...
                break :blk conf_item orelse .RCC_SWPMI1CLKSOURCE_PCLK;
            };

            std.mem.doNotOptimizeAway(SWPMI1CLockSelectionValue);
            ...
            const PLLPoutputFreq_ValueValue: ?f32 = blk: {
                if (((SAI1SourcePLLP and (config.flags.SAI1Used_ForRCC))) and scale1) {
                    PLLPoutputFreq_ValueLimit = .{
                        .min = 2.064e6,
                        .max = 8e7,
                    };
                    break :blk null;
                } else if (((SAI1SourcePLLP and (config.flags.SAI1Used_ForRCC))) and scale2) {
                    PLLPoutputFreq_ValueLimit = .{
                        .min = 2.064e6,
                        .max = 2.6e7,
                    };
            ...
            const PLLUsedValue: ?f32 = blk: {
                if (((SAI1SourcePLLP and (config.flags.SAI1Used_ForRCC))) or (SysSourcePLL) or ((check_ref(@TypeOf(RCC_MCO1SourceValue), RCC_MCO1SourceValue, .RCC_MCO1SOURCE_PLLCLK, .@"=")) and ((check_MCU("Semaphore_input_Channel1TIM16") and check_MCU("TIM16") and check_MCU("SEM2RCC_MCO_REQUIRED_TIM16")) or config.flags.MCOConfig)) or (CK48SourcePLLCLK and (config.flags.USBUsed_ForRCC or config.flags.RNGUsed_ForRCC or config.flags.SDMMC1Used_ForRCC))) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            const PLLSAI1UsedValue: ?f32 = blk: {
                if (((SAI1SourcePLLSAI1P and (config.flags.SAI1Used_ForRCC))) or (ADCSourcePLLSAI1R and (config.flags.USE_ADC1 or config.flags.USE_ADC2 or config.flags.USE_ADC3)) or (CK48SourcePLLSAI1 and (config.flags.USBUsed_ForRCC or config.flags.RNGUsed_ForRCC or config.flags.SDMMC1Used_ForRCC))) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            std.mem.doNotOptimizeAway(SWPMIEnableValue);
            ...
            if (check_ref(@TypeOf(SWPMIEnableValue), SWPMIEnableValue, .true, .@"=")) {
                const SWPMIMult_clk_value = SWPMI1CLockSelectionValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            ref_out.SWPMIEnable = SWPMIEnableValue;
