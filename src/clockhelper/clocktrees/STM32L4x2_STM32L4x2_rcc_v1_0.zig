    ...
    return struct {
        ...
        pub const CK48CLockSelectionList = enum {
            ...
            pub fn to_enum(self: @This()) anyerror!RCC_CLK48SEL {
                return switch (self) {
                    .RCC_USBCLKSOURCE_PLL => .PLLSAI1_Q,
            ...
            pub fn from_enum(item: RCC_CLK48SEL) anyerror!@This() {
                return switch (item) {
                    .PLLSAI1_Q => .RCC_USBCLKSOURCE_PLL,
        ...
        pub fn get_cubemx_clocks(config: CubeMXConfig, comptime patch_logs: bool) anyerror!CubemxTreeOutput {
                    ...
                    if (user_val) |val| {
                        switch (val) {
                            ...
                            .RCC_PLLSOURCE_MSI => PLLSourceMSI = true,
                                ...
                                , .{
                                    "PLLSourceVirtual",
                                    "(USBUsed_ForRCC & (CK48SourcePLLCLK)) ",
            ...
            const PLLUsedValue: u1 = blk: {
                if ((SysSourcePLL) or ((false) and ((config.flags.Semaphore_input_Channel1TIM16 and config.flags.TIM16 and config.flags.SEM2RCC_MCO_REQUIRED_TIM16) or config.flags.MCOConfig)) or (CK48SourcePLLCLK and (config.flags.USBUsed_ForRCC or config.flags.RNGUsed_ForRCC))) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            const MSIAutoCalibrationValue: MSIAutoCalibrationList = blk: {
                if (config.flags.USBUsed_ForRCC and (CK48SourceMSI or (CK48SourcePLLCLK and PLLSourceMSI) or (PLLSourceMSI)) and (config.flags.LSEOscillator or config.flags.LSEByPass)) {
                    if (config.extra_config.MSIAutoCalibration) |val| {
                        if (val != .ENABLED) {
                            ...
                            , .{
                                "MSIAutoCalibration",
                                "USBUsed_ForRCC&(CK48SourceMSI|(CK48SourcePLLCLK&PLLSourceMSI)|(PLLSourceMSI))& (LSEOscillator | LSEByPass)",
                                "",
                                "ENABLED",
                                val,
                            });
            ...
            //======= CRSFreq_Value ========
            const CRSFreq_ValueValue = CRSOutput.get_as_ref();
            ...
            if (config.flags.USBUsed_ForRCC and (config.flags.USE_ADC1 or config.flags.USE_ADC2) and (!config.flags.ADC1UsedAsynchronousCLK_ForRCC or !config.flags.ADC2UsedAsynchronousCLK_ForRCC)) {
                AHBOutput.limit = .{
                    .min = 1.42e7,
                    .max = 8e7,
                    .main_expr = "USBUsed_ForRCC & (USE_ADC1|USE_ADC2) & (!ADC1UsedAsynchronousCLK_ForRCC|!ADC2UsedAsynchronousCLK_ForRCC)",
            ...
            if (((config.flags.USBUsed_ForRCC or config.flags.RNGUsed_ForRCC) and CK48SourcePLLCLK) and scale1) {
                PLLQoutput.limit = .{
                    .min = 8e6,
                    .max = 8e7,
                    .main_expr = "((USBUsed_ForRCC|RNGUsed_ForRCC)& CK48SourcePLLCLK) & scale1",
                    .main_dialog = " ",
                };
                ...
                PLLQoutput.limit = .{
                    .min = 8e6,
                    .max = 2.6e7,
                    .main_expr = "((USBUsed_ForRCC|RNGUsed_ForRCC)& CK48SourcePLLCLK) & scale2",
                    .main_dialog = " ",
                };
            }
