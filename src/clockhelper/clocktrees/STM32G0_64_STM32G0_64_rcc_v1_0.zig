    ...
    return struct {
        ...
        pub fn get_cubemx_clocks(config: CubeMXConfig, comptime patch_logs: bool) anyerror!CubemxTreeOutput {
            ...
            const TIM1EnableValue: TIM1EnableList = blk: {
                if (true and true and !check_MCU("STM32G0x0_Value_line")) {
                    break :blk .true;
                }
                break :blk .false;
            };
            ...
            const EnableExtClockForI2SValue: EnableExtClockForI2SList = blk: {
                if (config.flags.I2S1Used_ForRCC and config.flags.AudioClockConfig and true) {
                    break :blk .true;
                }
                break :blk .false;
            };
            ...
            if (scale1 and ((RNGClockisPll and config.flags.RNGUsed_ForRCC) or (TIM1SOURCEPll and config.flags.TIM1Used_ForRCC) or (TIM1SOURCEPll and true))) {
                PLLQoutput.limit = .{
                    .min = 1.2e7,
                    .max = 1.28e8,
                };
            ...
            const PLLUsedValue: u1 = blk: {
                if ((I2S1SourcePll and config.flags.I2S1Used_ForRCC) or (check_MCU("ADC1_USED") and ADCSourcePllp and config.flags.ADCUsed_ForRCC) or (true and TIM1SOURCEPll) or (SysSourcePLL) or ((check_ref(@TypeOf(RCC_MCO1SourceValue), RCC_MCO1SourceValue, .RCC_MCO1SOURCE_PLLCLK, .@"=")) and ((check_MCU("Semaphore_input_Channel1TIM17") and check_MCU("TIM17") and check_MCU("SEM2RCC_MCO_REQUIRED_TIM17")) or config.flags.MCOConfig)) or (RNGClockisPll and (config.flags.RNGUsed_ForRCC))) {
                    break :blk 1;
                }
                break :blk 0;
            };
