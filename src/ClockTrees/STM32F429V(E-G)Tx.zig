...
pub const PLLI2SDivQConf = enum(u32) {
    _,
    pub fn get(num: @This()) f32 {
        const val: u32 = @intFromEnum(num);
        return @as(f32, @floatFromInt(val));
...
pub const SAIClockSource_BlockAConf = enum {
...
pub const PLLSAIDivQConf = enum(u32) {
    _,
    pub fn get(num: @This()) f32 {
        const val: u32 = @intFromEnum(num);
        return @as(f32, @floatFromInt(val));
...
pub const SAIClockSource_BlockBConf = enum {
    SAI_CLKSOURCE_PLLI2S,
    SAI_CLKSOURCE_EXT,
    SAI_CLKSOURCE_PLLSAI,
...
pub const Config = struct {
    ...
    PLLI2SSAIDiv: ?PLLI2SDivQConf = null,
    SAI_AMuxSrc: ?SAIClockSource_BlockAConf = null,
    PLLSAILCDDiv: ?PLLSAIDivRConf = null,
    PLLSAISAIDiv: ?PLLSAIDivQConf = null,
    SAI_BMuxSrc: ?SAIClockSource_BlockBConf = null,
...
pub const ConfigWithRef = struct {
    ...
    PLLI2SDivQ: ?PLLI2SDivQConf = null,
    SAIClockSource_BlockA: ?SAIClockSource_BlockAConf = null,
    PLLSAIDivR: ?PLLSAIDivRConf = null,
    PLLSAIDivQ: ?PLLSAIDivQConf = null,
    SAIClockSource_BlockB: ?SAIClockSource_BlockBConf = null,
    ...
    pub fn into_config(self: *const ConfigWithRef) Config {
        return .{
            ...
            .PLLI2SSAIDiv = self.PLLI2SDivQ,
            .SAI_AMuxSrc = self.SAIClockSource_BlockA,
            .PLLSAILCDDiv = self.PLLSAIDivR,
            .PLLSAISAIDiv = self.PLLSAIDivQ,
            .SAI_BMuxSrc = self.SAIClockSource_BlockB,
...
pub const ClockTree = struct {
    ...
    pub fn init(comptime config: Config) Self {
        ...
        const PLLI2SSAIDivval = ClockNodeTypes{
            .div = .{
                .value = if (config.PLLI2SSAIDiv) |val| val.get() else 1,
                .limit = .{ .max = @min(1_000_000_000, 32), .min = 1 },
            },
        };
        const PLLI2SSAIDiv: ClockNode = .{
            .name = "PLLI2SSAIDiv",
            .nodetype = PLLI2SSAIDivval,
            .parents = &[_]*const ClockNode{&PLLI2SQ},
        };
        const PLLSAINval = ClockNodeTypes{
            .mul = .{
                .value = if (config.PLLSAIN) |val| val.get() else 49,
                .limit = .{ .max = @min(1_000_000_000, 432), .min = 50 },
            },
        };
        ...
        const PLLSAISAIDivval = ClockNodeTypes{
            .div = .{
                .value = if (config.PLLSAISAIDiv) |val| val.get() else 1,
                .limit = .{ .max = @min(1_000_000_000, 32), .min = 1 },
            },
        };
        const PLLSAISAIDiv: ClockNode = .{
            .name = "PLLSAISAIDiv",
            .nodetype = PLLSAISAIDivval,
            .parents = &[_]*const ClockNode{&PLLSAIQ},
        };
        const SAI_AMuxSrcval = ClockNodeTypes{
            .multi = inner: {
                if (config.SAI_AMuxSrc) |val| {
                    break :inner val.get();
                } else {
                    break :inner 2;
                }
        ...
        const SAI_ACLKOutputval = ClockNodeTypes{ .output = null };
        const SAI_ACLKOutput: ClockNode = .{
            .name = "SAI_ACLKOutput",
            .nodetype = SAI_ACLKOutputval,
        ...
        const PLLSAILCDDivval = ClockNodeTypes{ .div = .{
            .value = inner: {
                if (config.PLLSAILCDDiv) |val| {
                    break :inner val.get();
                } else {
                    break :inner 2;
                }
        ...
        const SAI_BMuxSrcval = ClockNodeTypes{
            .multi = inner: {
                if (config.SAI_BMuxSrc) |val| {
                    break :inner val.get();
                } else {
                    break :inner 2;
                }
        ...
        const SAI_BCLKOutputval = ClockNodeTypes{ .output = null };
        const SAI_BCLKOutput: ClockNode = .{
            .name = "SAI_BCLKOutput",
            .nodetype = SAI_BCLKOutputval,
    ...
    pub fn init_runtime_tree(self: *Self, alloc: std.mem.Allocator) !void {
        ...
        self.PLLI2SSAIDiv.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLI2SQ,
        });
        self.SAI_AMuxSrc.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLI2SSAIDiv,
            &self.I2S_CKIN,
            &self.PLLSAISAIDiv,
        });
        self.SAI_ACLKOutput.parents = try alloc.dupe(*const ClockNode, &.{
            &self.SAI_AMuxSrc,
        });
        self.PLLSAILCDDiv.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSAIR,
        });
        self.@"LCD-TFTCLKOutput".parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSAILCDDiv,
        });
        self.PLLSAISAIDiv.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLSAIQ,
        });
        self.SAI_BMuxSrc.parents = try alloc.dupe(*const ClockNode, &.{
            &self.PLLI2SSAIDiv,
            &self.I2S_CKIN,
            &self.PLLSAISAIDiv,
        });
        self.SAI_BCLKOutput.parents = try alloc.dupe(*const ClockNode, &.{
            &self.SAI_BMuxSrc,
        });
    ...
    pub fn deinit_runtime_tree(self: *Self, alloc: std.mem.Allocator) void {
        ...
        alloc.free(self.PLLI2SSAIDiv.parents.?);
        alloc.free(self.SAI_AMuxSrc.parents.?);
        alloc.free(self.SAI_ACLKOutput.parents.?);
        alloc.free(self.PLLSAILCDDiv.parents.?);
        alloc.free(self.@"LCD-TFTCLKOutput".parents.?);
        alloc.free(self.PLLSAISAIDiv.parents.?);
        alloc.free(self.SAI_BMuxSrc.parents.?);
        alloc.free(self.SAI_BCLKOutput.parents.?);
    ...
    pub fn runtime_apply(self: *Self, config: Config) error{InvalidConfig}!void {
        ...
        self.PLLI2SSAIDiv.nodetype = PLLI2SSAIDivval;
        const PLLSAINval = ClockNodeTypes{
            .mul = .{
                .value = if (config.PLLSAIN) |val| val.get() else 49,
                .limit = .{ .max = @min(1_000_000_000, 432), .min = 50 },
        ...
        self.PLLSAISAIDiv.nodetype = PLLSAISAIDivval;
        const SAI_AMuxSrcval = ClockNodeTypes{
            .multi = inner: {
                if (config.SAI_AMuxSrc) |val| {
                    break :inner val.get();
        ...
        self.SAI_AMuxSrc.nodetype = SAI_AMuxSrcval;
        const SAI_ACLKOutputval = ClockNodeTypes{ .output = null };
        self.SAI_ACLKOutput.nodetype = SAI_ACLKOutputval;
        const PLLSAIRval = ClockNodeTypes{
            .div = .{
                .value = if (config.PLLSAIR) |val| val.get() else 2,
                .limit = .{ .max = @min(1_000_000_000, 7), .min = 2 },
        ...
        self.PLLSAILCDDiv.nodetype = PLLSAILCDDivval;
        const @"LCD-TFTCLKOutputval" = ClockNodeTypes{ .output = null };
        ...
        self.SAI_BMuxSrc.nodetype = SAI_BMuxSrcval;
        const SAI_BCLKOutputval = ClockNodeTypes{ .output = null };
        self.SAI_BCLKOutput.nodetype = SAI_BCLKOutputval;
        const MCO1Multval = ClockNodeTypes{
            .multi = inner: {
                if (config.MCO1Mult) |val| {
                    break :inner val.get();
