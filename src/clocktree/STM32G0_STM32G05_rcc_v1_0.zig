    ...
    return struct {
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            const PLLPoutputFreq_ValueValue: ?f32 = blk: {
                if (scale1 and ((check_MCU("ADC1_USED") and config.flags.ADCUsed_ForRCC and ADCSourcePllp) or (I2S1SourcePll and config.flags.I2S1Used_ForRCC))) {
            ...
            const PLLQoutputFreq_ValueValue: ?f32 = blk: {
                if (scale1 and ((RNGClockisPll and config.flags.RNGUsed_ForRCC) or (TIM1SOURCEPll and config.flags.TIM1Used_ForRCC) or (TIM15SOURCEPll and check_MCU("TIM15")) or (TIM1SOURCEPll and check_MCU("TIM1")))) {
                    ...
                    if (conf_item) |i| {
                        if (item != i) {
                            try comptime_fail_or_error(error.InvalidConfig,
                                ...
                                \\select the expected option or leave the value as null.
                                \\
                            , .{ "FLatency", "((HCLKFreq_Value < 24000000)|(HCLKFreq_Value= 24000000 ))", "No Extra Log", "FLASH_LATENCY_0", i });
                        }
                    }
                    break :blk item;
                    ...
                    if (conf_item) |i| {
                        if (item != i) {
                            try comptime_fail_or_error(error.InvalidConfig,
                                ...
                                \\select the expected option or leave the value as null.
                                \\
                            , .{ "FLatency", "((HCLKFreq_Value < 48000000)|(HCLKFreq_Value= 48000000))", "No Extra Log", "FLASH_LATENCY_1", i });
                        }
                    }
                    break :blk item;
                } else if (((check_ref(@TypeOf(HCLKFreq_ValueValue), HCLKFreq_ValueValue, 64000000, .@"<")) or (check_ref(@TypeOf(HCLKFreq_ValueValue), HCLKFreq_ValueValue, 64000000, .@"=")))) {
                    ...
                    if (conf_item) |i| {
                        if (item != i) {
                            try comptime_fail_or_error(error.InvalidConfig,
                                ...
                                \\select the expected option or leave the value as null.
                                \\
                            , .{ "FLatency", "((HCLKFreq_Value < 64000000)|(HCLKFreq_Value= 64000000 ))", "No Extra Log", "FLASH_LATENCY_2", i });
                        }
                    }
                    break :blk item;
                }
            ...
            const PLLUsedValue: ?f32 = blk: {
                if ((I2S1SourcePll and config.flags.I2S1Used_ForRCC) or (check_MCU("ADC1_USED") and ADCSourcePllp and config.flags.ADCUsed_ForRCC) or (check_MCU("TIM15") and TIM15SOURCEPll) or (check_MCU("TIM1") and TIM1SOURCEPll) or (SysSourcePLL) or ((check_ref(@TypeOf(RCC_MCO1SourceValue), RCC_MCO1SourceValue, .RCC_MCO1SOURCE_PLLCLK, .@"=")) and ((check_MCU("Semaphore_input_Channel1TIM17") and check_MCU("TIM17") and check_MCU("SEM2RCC_MCO_REQUIRED_TIM17")) or config.flags.MCOConfig)) or (RNGClockisPll and (config.flags.RNGUsed_ForRCC))) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            if (!check_MCU("STM32G0x0_Value_line") and !check_MCU("DIE456")) {
