...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub const Config_Output = struct {
            ...
            HSI_DIV8: ?f32 = null, //from RCC Clock Config
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            const HSI_DIV8Value: ?f32 = blk: {
                break :blk 8;
            };

            std.mem.doNotOptimizeAway(HSI_DIV8Value);
            ...
            const PLLQoutputFreq_ValueValue: ?f32 = blk: {
                if (scale1 and ((RNGClockisPll and config.flags.RNGUsed_ForRCC) or (TIM1SOURCEPll and config.flags.TIM1Used_ForRCC) or (TIM1SOURCEPll and check_MCU("TIM1")))) {
                        ...
                        switch (item) {
                            ...
                            .FLASH_LATENCY_2 => {},
                                ...
                                try comptime_fail_or_error(error.InvalidConfig,
                                    ...
                                    \\ - FLASH_LATENCY_1
                                    \\ - FLASH_LATENCY_2
                                , .{ "FLatency", "scale1 & ((HCLKFreq_Value < 48000000)|(HCLKFreq_Value= 48000000))", "No Extra Log", item });
                            },
                        }
                    }
            ...
            const PLLUsedValue: ?f32 = blk: {
                if ((I2S1SourcePll and config.flags.I2S1Used_ForRCC) or (check_MCU("ADC1_USED") and ADCSourcePllp and config.flags.ADCUsed_ForRCC) or (check_MCU("TIM1") and TIM1SOURCEPll) or (SysSourcePLL) or ((check_ref(@TypeOf(RCC_MCO1SourceValue), RCC_MCO1SourceValue, .RCC_MCO1SOURCE_PLLCLK, .@"=")) and ((check_MCU("Semaphore_input_Channel1TIM17") and check_MCU("TIM17") and check_MCU("SEM2RCC_MCO_REQUIRED_TIM17")) or config.flags.MCOConfig)) or (RNGClockisPll and (config.flags.RNGUsed_ForRCC))) {
                    break :blk 1;
                }
                break :blk 0;
            };
            ...
            const TIM1EnableValue: ?TIM1EnableList = blk: {
                if (check_MCU("TIM1") and check_MCU("CodegenConfigPeriph") and !check_MCU("STM32G0x0_Value_line")) {
            ...
            if (check_MCU("RNG_Exist")) {
                const HSI_DIV8_clk_value = HSI_DIV8Value orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
                ...
                HSI_DIV8.nodetype = .div;
                HSI_DIV8.value = HSI_DIV8_clk_value;
                HSI_DIV8.parents = &.{&HSIRC};
            }
            ...
            if (!check_MCU("STM32G0x0_Value_line")) {
                if (check_ref(@TypeOf(RNGPLLEnableValue), RNGPLLEnableValue, .true, .@"=") or
                    check_ref(@TypeOf(TIM1PLLEnableValue), TIM1PLLEnableValue, .true, .@"="))
                {
            ...
            out.HSI_DIV8 = try HSI_DIV8.get_output();
            ...
            ref_out.HSI_DIV8 = HSI_DIV8Value;
