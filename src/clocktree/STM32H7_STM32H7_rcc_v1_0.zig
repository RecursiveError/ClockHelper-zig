...
pub fn ClockTree(comptime mcu_data: std.StaticStringMap(void)) type {
    return struct {
        ...
        pub fn get_clocks(config: Config) anyerror!Tree_Output {
            ...
            std.mem.doNotOptimizeAway(RC48_VALUEValue);
            ...
            std.mem.doNotOptimizeAway(ProductRevValue);
            ...
            std.mem.doNotOptimizeAway(Cortex2_DivValue);
            ...
            std.mem.doNotOptimizeAway(SAI23CLockSelectionValue);
            ...
            std.mem.doNotOptimizeAway(SAI23EnableValue);
            ...
            std.mem.doNotOptimizeAway(I2C123EnableValue);
            ...
            const RC48_clk_value = RC48_VALUEValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                \\Error on Clock {s} | expr: {s} diagnostic: {s}
                \\Clock is active but the reference value {s} is null
                \\note: check the flags and configurations associated with this clock
                \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_MCU("STM32H745_755") or check_MCU("STM32H747_757") or check_MCU("STM32H7x5")) {
                const Cortex2Prescaler_clk_value = Cortex2_DivValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
            ...
            if (check_ref(@TypeOf(SAI23EnableValue), SAI23EnableValue, .true, .@"=")) {
                const SAI23Mult_clk_value = SAI23CLockSelectionValue orelse try comptime_fail_or_error(error.InvalidClockValue,
                    \\Error on Clock {s} | expr: {s} diagnostic: {s}
                    \\Clock is active but the reference value {s} is null
                    \\note: check the flags and configurations associated with this clock
                    \\note2: actually I'm not sure if null values and clock nodes should be treated as errors.
