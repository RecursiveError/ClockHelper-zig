...
pub const ClockTree = struct {
    ...
    pub fn init(comptime config: Config) Self {
        ...
        const SysCLKOutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 480000000), .min = 0 },
        };
        ...
        const VCOoutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 2000000000), .min = 1000000000 },
        };
        ...
        const DSIoutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 62000000), .min = 0 },
        };
        ...
        const PLLDSIoutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 1000000000), .min = 62500000 },
        };
        ...
        const AHBOutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 240000000), .min = 0 },
        };
        ...
        const RTCOutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 1000000), .min = 0 },
        };
        ...
        const USBoutputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 66000000), .min = 0 },
        };
    ...
    pub fn init_runtime_tree(self: *Self, alloc: std.mem.Allocator) !void {
        ...
        self.SAI23Mult.parents = try alloc.dupe(*const ClockNode, &.{
            &self.DIVQ1,
            &self.DIVP2,
            &self.DIVP3,
            &self.I2S_CKIN,
        ...
        self.SAI23output.parents = try alloc.dupe(*const ClockNode, &.{
            &self.SAI23Mult,
        });
    ...
    pub fn deinit_runtime_tree(self: *Self, alloc: std.mem.Allocator) void {
        ...
        alloc.free(self.SAI23Mult.parents.?);
        alloc.free(self.SAI23output.parents.?);
    ...
    pub fn runtime_apply(self: *Self, config: Config) error{InvalidConfig}!void {
        ...
        self.SAI23Mult.nodetype = SAI23Multval;
        const SAI23outputval = ClockNodeTypes{
            .output = .{ .max = @min(1_000_000_000, 150000000), .min = 0 },
        };
        self.SAI23output.nodetype = SAI23outputval;
        const SAI1Multval = ClockNodeTypes{
            .multi = inner: {
                if (config.SAI1Mult) |val| {
                    break :inner val.get();
